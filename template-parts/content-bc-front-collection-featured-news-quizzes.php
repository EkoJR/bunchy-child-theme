...
$news_category = get_category_by_slug( 'featured-news-quizzes' );

$args = array(
	'post_type'  => ['snax_quiz', 'post'],
...
<div class="bc-frontpage-01">
	<div class="bc-fp-01">
		<div class="respon_row respon_group bc-fp-01-l-top">
			...
			?>
			<?php while ( $query->have_posts() && ( $total_columns - 1 ) > $post_index ) : ?>
				<div class="respon_col span_1_of_2">
					<div class="respon_row respon_group">
						<?php while ( 0 < ( $total_column - $post_span ) || ( 1 === $total_column && $post_span === $total_column ) ) : $query->the_post(); ?>
							<div class="bc-fp-01-post-container respon_col span_<?php echo $post_span; ?>_of_2">
								<article id="post-<?php $query->the_ID(); ?>" <?php post_class( 'bc-fp-01-post' ); ?> itemscope="" itemtype="<?php echo esc_attr( bunchy_get_entry_microdata_itemtype() ); ?>">
									...
									<?php if ( $bunchy_elements['title'] || ( $show_subtitle && bunchy_can_use_plugin( 'wp-subtitle/wp-subtitle.php' ) ) ) : ?>
										<header>
											...
											<?php endif; ?>
											<?php
											if ( isset( $show_subtitle ) && $show_subtitle && bunchy_can_use_plugin( 'wp-subtitle/wp-subtitle.php' ) ) {
		...
		<div class="respon_row respon_group bc-fp-01-l-bottom">
			...
			<?php while ( $query->have_posts() && ( $total_columns - 1 ) > $post_index ) : ?>
				<div class="respon_col span_1_of_2">
					<div class="respon_row respon_group">
						<?php while ( 0 < ( $total_column - ( $post_span + 1 ) ) || ( 2 === $total_column && $post_span === $total_column ) ) : $query->the_post(); ?>
							<div class="bc-fp-01-post-container respon_col span_<?php echo $post_span; ?>_of_2">
								<article id="post-<?php $query->the_ID(); ?>" <?php post_class( 'bc-fp-01-post' ); ?> itemscope="" itemtype="<?php echo esc_attr( bunchy_get_entry_microdata_itemtype() ); ?>">
									<?php the_post_thumbnail( 'medium' ); ?>
									<?php if ( $bunchy_elements['title'] || ( $show_subtitle && bunchy_can_use_plugin( 'wp-subtitle/wp-subtitle.php' ) ) ) : ?>
...
<a class="g1-button g1-button-m g1-button-solid snax-button snax-button-create" href="https://internettop40.com/frontend-submission-2/">Create</a>
<br style="clear: both;">
<?php
$wp_query = $temp_query;
